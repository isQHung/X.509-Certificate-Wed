export default function UserCertificatesPage() {
  const myCerts = [
    { id: 1, cn: "localhost.dev", serial: "77:AF:22:...", expiry: "2027-04-01", status: "Active" }
  ];
          ...
          <tbody className="divide-y divide-slate-100">
            {myCerts.map(cert => (
              <tr key={cert.id} className="hover:bg-slate-50/50 transition">
                <td className="p-4 font-bold text-slate-700">{cert.cn}</td>
                <td className="p-4 font-mono text-xs text-slate-500">{cert.serial}</td>
                <td className="p-4 text-sm text-slate-600">{cert.expiry}</td>
